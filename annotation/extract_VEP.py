...
for i in open(sys.argv[1],"r"):
	...
	if retrieved != "empty":
		groups[group] = groups[group]+","+sample
	if retrieved == "empty":
		groups[group] = sample
...
first=0
extra_fields=[]
genotype=dict([]) #genotype columns (per person)
gcolumns=dict([]) #group columns
for i in open(sys.argv[2],"r"):
	...
	if s[0]=="#CHROM":
		...
		for j in groups.keys():
			this = []
			for k in groups[j].split(","):
				this.append(genotype[k])
			gcolumns[j]=this
			for k in count_labels:
				counts_header.append(j+"_"+k)
	...
	if s[0][0]!="#":
		...
		csq=[]
		csq_out=[]
		extra_n=0 #position of 'extra' information in info (alters if CSQ present)
		if info[0][0:3]=="CSQ":
			extra_n=1
